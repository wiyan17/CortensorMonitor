...
def auto_update(context: CallbackContext):
    """
    Job function to send an auto-update message with combined node status, health, and stall info.
    """
    job = context.job
...
def alert_check(context: CallbackContext):
    """
    Job function to check for alerts and send alert messages if needed.
    """
    job = context.job
    chat_id = job.context['chat_id']
    addresses = get_addresses_for_chat(chat_id)[:10]
...
def auto_node_stall(context: CallbackContext):
    """
    Job function to send a concise update for node stall status.
    """
    job = context.job
    chat_id = job.context['chat_id']
    addresses = get_addresses_for_chat(chat_id)[:10]
